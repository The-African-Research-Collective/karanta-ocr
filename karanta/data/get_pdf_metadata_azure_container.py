...
async def process_blob_pdf(container: ContainerClient, blob_name: str, model: str, prompt_template: dict, sem: Semaphore, queue: asyncio.Queue):
    async with sem:
        try:
...
async def concurrent_stream_process(container_name: str, model: str, max_concurrent_pdfs: int, output: str, prompt_template: dict):
    ...
    async with container:
        ...
        async for blob in container.list_blobs():
            if blob.name.lower().endswith(".pdf"):
                tasks.append(asyncio.create_task(process_blob_pdf(container, blob.name, model, prompt_template, sem, queue)))
