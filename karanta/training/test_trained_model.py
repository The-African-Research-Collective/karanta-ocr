...
def load_prompt_from_yaml(yaml_path: str, key: str) -> str:
    ...
    if key not in data:
        raise KeyError(f"Prompt key '{key}' not found in {yaml_path}. Available keys: {list(data.keys())}")
    return data[key]
...
def build_message(image_url: str, system_prompt: str, page: int = 0):
    ...
    print(
        f"Prompt: {prompt_template_dict.render({'base_text': anchor_text})}\n======="
    )
...
def main():
    ...
    parser.add_argument("--model_path", type=str, required=True, help="Path to trained or base model")
    parser.add_argument("--processor_name", type=str, default="Qwen/Qwen2.5-VL-3B-Instruct")
    parser.add_argument("--image", type=str, required=True, help="Image URL or path to test")
    parser.add_argument("--prompt_yaml", type=str, required=True, help="Path to YAML file containing prompt configs")
    parser.add_argument("--prompt_key", type=str, required=True, help="Key inside YAML file to use (e.g. system, olmo_ocr_system_prompt)")
    ...
    # Run inference
    output = run_inference(model, processor, messages, max_new_tokens=args.max_new_tokens, device=args.device)
